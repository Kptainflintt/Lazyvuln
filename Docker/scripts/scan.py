...
def execute_command(command: str, xpath: Optional[str] = None) -> Union[str, float, bool, List]:
    ...
    command: str = "su - service -c \"gvm-cli --gmp-username admin --gmp-password admin " \
                   "socket --socketpath /run/gvmd/gvmd.sock --xml \'{}\'\"".format(command)
...
def start_scan(args: argparse.Namespace) -> None:
    ...
    subprocess.check_call(
        ["sed -i 's/max_hosts.*/max_hosts = " + str(args.hosts) + "/' /etc/openvas/openvas.conf"],
        shell=True,
        stdout=subprocess.DEVNULL
    )
    subprocess.check_call(
        ["sed -i 's/max_checks.*/max_checks = " + str(args.checks) + "/' /etc/openvas/openvas.conf"],
        shell=True,
        stdout=subprocess.DEVNULL
    )
    ...
    if args.update is True:
        ...
        subprocess.check_call(["update-scanner"], shell=True)
    else:
        print("Starting OpenVAS...")
        subprocess.check_call(["start-scanner"], shell=True)
